...
public class DialogueInstanceEditor : Editor 
{
    ...
    void ShowEntryEditor (int i) 
    {
        ...
        {
                int k = 0;
                foreach (var n in dialogue[i].narration)
            GUILayout.BeginHorizontal();
            GUILayout.Box(""+k);
    ...
    {
        var dialogue = target.dialogue;
        Array arr = new Array(dialogue[i].links);
        arr.RemoveAt(j);
        dialogue[i].links = arr.ToBuiltin(Link);
    }
    ...
    {
        var dialogue = target.dialogue;
        Array arr = new Array(dialogue[i].choices);
        arr.RemoveAt(j);
        dialogue[i].choices = arr.ToBuiltin(DialogueChoice);
    }
    ...
    {
        var dialogue = target.dialogue;
        Array arr = new Array(dialogue[i].passwords);
        arr.RemoveAt(j);
    ...
    void GoBack () 
    {
        ...
        Array arr = new Array(target.history);
        arr.Pop();
    ...
    {
        var dialogue = target.dialogue;
        Array arr = new Array(dialogue[i].narration);
        if (target.defaultAudio) 
        {
